...
from tgbot_client.exception import FileNotFound, NoThisUserInGroup
...
class ApiManager:
    ...
    def __init__(self) -> None:
        self.com_api = tguserApi()
        self.file_manager = None

    async def init(self, file_manager: FileManager, config: Config) -> None:
        """
        初始化com
        """
...
class ActionManager(ApiManager):
    ...
    @standard_action
    async def get_status(self) -> ActionResponse:
        """
        获取运行状态
        """
        user_info = await self.get_self_info()
        bot = {
    ...
    @standard_action
    async def get_self_info(self) -> ActionResponse:
        """
        获取机器人自身信息
        """
        info = await self.com_api.get_self_info()
        return ActionResponse(status="ok", retcode=0, data=info)
    ...
    def get_user_info(self, user_id: str) -> ActionResponse:
        ...
        data = {
            ...
            f"{PREFIX}.wx_number": info["wxNumber"],  # TG号
        ...
        for one in members:
            if one["wxId"] == user_id:
                data = {
                    ...
                    f"{PREFIX}.wx_number": one["wxNumber"],  # TG号
    ...
    @expand_action
    def get_tgchat_version(self) -> ActionResponse:
        """
        获取TG版本
        """
        data = self.com_api.get_tgchat_version()
        return ActionResponse(status="ok", retcode=0, data=data)

    @expand_action
    def set_tgchat_version(self, version: str) -> ActionResponse:
        """
        说明:
        ...
        """
        status = self.com_api.change_tgchat_version(version)
        if status:
            return ActionResponse(status="ok", retcode=0, data=None)
        else:
            return ActionResponse(
