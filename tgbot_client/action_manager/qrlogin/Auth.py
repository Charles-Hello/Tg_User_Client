...
class Auth:
    ...
    def _login_by_qrcode(self):
        ...
        if self._port or self._email:
            if self._port:
                ...
                self.log.info(f"请访问 http://127.0.0.1:{self._port} 扫描二维码")
                _thread.start_new_thread(self._show_qrcode_in_web, (tglogin_name,))
            if self._email:
                if self._email != ("", ""):
                    self._send_email(tglogin_name)
        else:
            qrcode_png = self._show(tglogin_name)
            if qrcode_png:
    ...
    def _show_qrcode_in_web(self, filename: str):
        ...
        self._webServer = HTTPServer(("127.0.0.1", int(self._port)), LoginServer)
