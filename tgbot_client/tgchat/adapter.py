...
from tgbot_client.consts import IMPL, ONEBOT_VERSION, USER_AGENT, VERSION
from tgbot_client.driver import (
    URL,
    BackwardWebSocket,
    Driver,
    FastAPIWebSocket,
...
from tgbot_client.exception import WebSocketClosed
from tgbot_client.onebot12 import ConnectEvent, Event, StatusUpdateEvent
from tgbot_client.utils import DataclassEncoder, escape_tag, logger_wrapper
        ...
        while True:
            try:
                async with self.start_websocket(setup) as websocket:
                    ...
                    # 发送update事件
                    event = self.get_status_update_event()
                    c = event.json(ensure_ascii=False, cls=DataclassEncoder)
                    print(c)
                    try:
                        await websocket.send(
                            event.json(ensure_ascii=False, cls=DataclassEncoder)
